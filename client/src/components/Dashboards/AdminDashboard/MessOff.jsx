...
function MessOff() {
  ...
  const updateRequest = async (id, status) => {
    const res = await fetch("https://hostel-management-and-booking-systems.onrender.com/api/messoff/update", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const graph = (
    <div className="w-96 h-52 bg-white rounded-xl shadow-xl p-5">
      <Bar
        data={{
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center justify-center bg-white">
      ...
      <h1 className="text-black font-bold text-5xl">Manage Mess</h1>
      <div className="w-96 h-52">{graph}</div>
      <div className="bg-white px-10 py-5 rounded-xl shadow-xl sm:w-[50%] sm:min-w-[450px] w-full mt-5 max-h-96 overflow-auto">
        <span className="text-black font-bold text-xl">All Requests</span>
        <ul role="list" className="divide-y divide-gray-200 text-black">
          {newReqs.length === 0 ? (
            <li className="mt-2 text-black">No new requests</li>
          ) : (
            newReqs.map((req) => (
              <li
