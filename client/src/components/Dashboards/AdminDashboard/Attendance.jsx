...
function Attendance() {
  const getALL = async () => {
    setProgress(30);
    const marked = await fetch("https://hostel-management-and-booking-systems.onrender.com/api/attendance/getHostelAttendance", {
      method: "POST",
      headers: {
  ...
  const markAttendance = async (id, isPresent) => {
    const data = await fetch(`https://hostel-management-and-booking-systems.onrender.com/api/attendance/mark`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const graph = (
    <div className="flex flex-row-reverse items-center gap-3 h-64 bg-white rounded-xl shadow-xl p-5">
      <Doughnut
        datasetIdKey="id"
        data={{
          labels,
  ...
  return (
    <div className="w-full h-screen flex flex-col gap-3 items-center xl:pt-0 md:pt-40 pt-64 justify-center overflow-auto max-h-screen bg-white">
